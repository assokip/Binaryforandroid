    ...
    app.events = {
        listeners : {
            ...
            add : function(event, fn) {
                ...
                if (app.debug.get()) app.events.dispatch('core.log.append','EVENTS:ADD:'+event);
            },
            remove : function(event, fn) {
                if (! this.list[event]) return;
                for (var i=0, l=this.list[event].length; i<l; i++) {
                    if (this.list[event][i] === fn) {
                        if (app.debug.get()) app.events.dispatch('core.log.append','EVENTS:REMOVE:'+event);
        ...
        dispatch : function(event, params) {
            ...
            for (var i=0, l=this.listeners.list[event].length; i<l; i++) {
                if (event !== 'core.log.append' && app.debug.get()) app.events.dispatch('core.log.append','EVENTS:DISPATCH:'+event);
