(function() {
	...
	document.querySelector('body >.wrapper >.loading').style.display='none';
		...
		mvm.getElementsByClassName('settings')[0].addEventListener('click', function() {
		    ...
		    tq.querySelector('.debug >.wrapper').className = 'wrapper ' + (app.core.debug.mode.get()? ' on' : ' off');
		    tq.querySelector('.cache >.wrapper').className = 'wrapper ' + (app.core.cache.mode.get()? ' on' : ' off');
		    tq.querySelector('.safety >.wrapper').className = 'wrapper ' + (app.binarycom.safety.mode.get()? ' on' : ' off');
		    if (! app.apigee.status.get()) tq.querySelector('.logout >.wrapper').className += ' disabled';
		...
		logout.addEventListener('click', function() {
		    if (app.apigee.status.get()) app.apigee.logout();
	    ...
	    // init 
		// previous login?
		var pl = app.apigee.status.get();
		if (pl) app.core.events.dispatch('core.status.append',{ title:'Credentials', lines : new Array('Using Previous Login: '+pl.login_id) });
