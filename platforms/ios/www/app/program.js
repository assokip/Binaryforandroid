window.addEventListener('load', function() {
    ...
    p.push({ name:'core' });
    p.push({ name:'jquery.min' });
    p.push({ name:'jquery.sparkline.min' });
    ...
    p.push({ name:'core.connection.xhr', type:'css' });
    p.push({ name:'core.connection.xhr' });
	p.push({ name:'core.form.message', type:'css' });
    p.push({ name:'core.form.message' });
    p.push({ name:'core.auth.openstandard.2' });
	...
	if (o.incompatible) { b = abort.querySelector('.incompatible'); }
		...
		b = abort.querySelector('.error');
		if (console.log) console.log(o.error.file.name);
	}
	...
	app['core.events'].listeners.add('core.debug.mode.set', function (o) {
	    ...
	    a.push(app['core.events'].listeners.add('core.auth.openstandard.2.login.request', function (o) {
		app['core.events'].dispatch('core.log.append',{ message:'CORE:OAUTH2:LOGIN:REQUEST: '+o.url, type:'debug' });
	    }));
	    a.push(app['core.events'].listeners.add('core.auth.openstandard.2.token.issued', function (o) {
	...
	app['core.events'].listeners.add('core.connection.exec.aborted', function(o) { f(o) });
	app['core.events'].listeners.add('core.connection.exec.success', function(o) { f(o) });
	app['core.events'].listeners.add('core.connection.exec.error', function (o) {
	    var s = o.status;
	    if (! s.nextto || ! s.container) return;
	    s.container.firstChild.className = 'error';
	    setTimeout(function() { f(o); },4000);
	});
	...
	app['binarycom.safety'].mode.set(app['core.store'].get({ id:'binarycom.safety.mode' }) !== false? true : false,true);
